...
export async function POST(request) {
  ...
  try {
    ...
    const excelFile = formData.get("publicationExcelFile");
    ...
    const publicationsToInsert = jsonData
      .map((row, index) => {
        ...
        if (!title || !authors || !journal) {
          console.warn(
            `Row ${
              index + 2
            } (Excel row number): Skipping due to missing Title or Authors or Journal. Data: ${JSON.stringify(
              row
            )}`
          );
          return null;
    ...
    if (publicationsToInsert.length === 0) {
      return NextResponse.json(
        {
          message:
            "No valid publication records found in the Excel sheet (Title and Authors and Journal are required for each row).",
        },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < publicationsToInsert.length; i++) {
      ...
      try {
        ...
        console.error(
          `Failed to insert record: ${JSON.stringify(publication)}`,
          dbError
        );
        errors.push({
          record: publication,
          error: dbError.message,
          excel_row: jsonData.findIndex((r) => r === publication) + 2,
        });
      }
    }
