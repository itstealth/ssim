...
const educationBackgroundData = {
  ug: [
    ...
    { label: "Economics & Commerce", percentage: 10.91 },
    { label: "Engineering & Technology", percentage: 4.72 },
    { label: "Business Management", percentage: 4.72 },
    { label: "Computer Science & Information Technology", percentage: 3.07 },
    { label: "Science", percentage: 1.71 },
  ],
  pg: [
    { label: "Business Management", percentage: 56.69 },
    { label: "Others", percentage: 43.31 },
  ],
};

const TopCareers = () => {
  const [level, setLevel] = useState("ug");
  const data = educationBackgroundData[level];
  ...
  return (
    <div className="bg-navy py-12 sm:py-20">
      <div className="container max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-none p-6 sm:px-10 sm:py-[50px] shadow-lg">
          <div className="relative flex items-center justify-center mb-10">
            <h3 className="font-playfair text-2xl sm:text-3xl font-bold uppercase tracking-wide text-gray-900 text-center">
              Top Careers
            </h3>
            <div className="absolute right-0 hidden sm:flex gap-2">
              <button
                type="button"
                onClick={() => setLevel("ug")}
                className={`px-4 py-1.5 rounded-none text-xs font-semibold transition-colors ${
                  level === "ug"
                    ? "bg-[#6C3EAA] text-white"
          ...
          <div className="flex sm:hidden gap-2 justify-center mb-8">
            <button
              type="button"
              onClick={() => setLevel("ug")}
              className={`px-4 py-1.5 rounded-none text-xs font-semibold transition-colors ${
          ...
          <div className="space-y-6">
            {data.map((item) => (
              <div key={item.label} className="flex items-center gap-4 sm:gap-6">
                <div className="w-28 sm:w-48 flex-shrink-0 text-right text-sm text-gray-800 leading-snug">
                  {item.label}
                </div>
                <div className="flex-1 h-9 sm:h-10 bg-[#F1F1F1]">
                  <div
                    className="h-full bg-[#6C3EAA] transition-all duration-700"
                    style={{ width: `${item.percentage}%` }}
                  />
                </div>
                <div className="w-10 flex-shrink-0 text-sm text-gray-800">
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`w-full flex items-center space-x-3 px-4 py-4 rounded-lg transition-all duration-200 text-left ${
                      activeTab === tab.id
                        ? "bg-purple-700 text-white font-medium shadow-md"
                        : "text-gray-600 hover:text-black hover:bg-gray-50"
                    }`}
                  >
                    <span
                      className={`${
                        activeTab === tab.id ? "text-white" : "text-purple-700"
                      }`}
                    >
                      {tab.icon}
                    </span>
